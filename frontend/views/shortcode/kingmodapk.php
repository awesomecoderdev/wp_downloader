    ...
    <?php if ($apps->have_posts()) : ?>
        <div class="my-3 grid lg:grid-cols-3 md:grid-cols-2 gap-3">
            <?php while ($apps->have_posts()) : $apps->the_post(); ?>
                <a class="max-w-md w-full relative bg-white flex rounded shadow overflow-hidden" href="<?php the_permalink(); ?>">
                    <img class="rounded h-20 w-20" width="120" height="120" src="<?php echo get_post_meta(get_the_ID(), "awesomecoder_app_icon", true); ?>" alt="<?php echo the_title(); ?>">
